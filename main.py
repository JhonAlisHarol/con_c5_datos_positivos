...
# --- 1. CONFIGURACIÓN DE SUPABASE (LLAVE ACTUALIZADA Y EN CACHÉ) ---
...
st.markdown("""
        ...
        /* Fix para hacer visibles los textos de los botones */
        div.stButton > button {
...
st.markdown(
    ...
    [data-testid="stSidebar"], div[data-testid="stVerticalBlock"] {
        background-color: rgba(10, 15, 25, 0.7) !important;
...
# --- 3. ESTADOS Y FUNCIONES AUXILIARES ---
...
if 'modo_pantalla' not in st.session_state: st.session_state.modo_pantalla = "NUEVO"
...
def parse_time(time_str):
    if not time_str or time_str == "None": return datetime.now().time()
    try: return datetime.strptime(time_str[:8], "%H:%M:%S").time()
    except: return datetime.now().time()

def parse_date(date_str):
    if not date_str or date_str == "None": return date.today()
...
if not st.session_state.autenticado:
    ...
    with st.sidebar:
        if st.session_state.get("autenticado", False):
            ...
            if st.button("✏️ Editar", use_container_width=True):
                st.session_state.modo_pantalla = "EDICION"
                st.rerun()
    ...
    # Añadir una key dinámica según el modo evita que el mapa colapse visualmente
    map_data = st_folium(m, height=500, width=1400, key=f"map_{st.session_state.modo_pantalla}")
    ...
    col_a.text_input("Latitud capturada", value=latitud_actual, disabled=True)
    col_b.text_input("Longitud capturada", value=longitud_actual, disabled=True)

    # --- 9. FORMULARIO MAESTRO ---
    st.divider()
    idx_modo = 0 if datos_cargados.get("MODO", "PREVENTIVO") == "PREVENTIVO" else 1
    modo = st.radio("SELECCIONE EL TIPO DE REGISTRO:", ["PREVENTIVO", "POSITIVO"], index=idx_modo, horizontal=True)
    st.divider()

    with st.form("registro_maestro_total", clear_on_submit=False):
        st.subheader("📍 Ubicación y Recursos")
        col_loc1, col_loc2, col_loc3 = st.columns(3)
        provincia = col_loc1.selectbox("PROVINCIA", lista_provincias, index=obtener_indice(lista_provincias, datos_cargados.get("PROVINCIA")))
        distrito = col_loc2.selectbox("DISTRITO", lista_distritos, index=obtener_indice(lista_distritos, datos_cargados.get("DISTRITO")))
        corregimiento = col_loc3.selectbox("CORREGIMIENTO", lista_corregimientos, index=obtener_indice(lista_corregimientos, datos_cargados.get("CORREGIMIENTO")))
        
        col_loc4, col_loc5, col_loc6 = st.columns(3)
        referencia = col_loc4.text_input("SECTOR/CALLE/AVENIDA", value=datos_cargados.get("REFERENCIA", ""))
        zp_policial = col_loc5.selectbox("ZP POLICIALES / ENLACE", lista_zp, index=obtener_indice(lista_zp, datos_cargados.get("ZP_POLICIAL")))
        recursos = col_loc6.selectbox("RECURSOS", lista_recursos, index=obtener_indice(lista_recursos, datos_cargados.get("RECURSOS")))
            
        st.subheader("⏱️ Tiempos, Unidades y Cámaras")
        c1, c2, c3, c4 = st.columns(4)
        fecha = c1.date_input("FECHA", value=parse_date(datos_cargados.get("FECHA_EVENTO")))
        centro_mando = c1.selectbox("CENTRO DE MANDO", ["SELECCIONAR", "CON-C5", "CORCOL", "COMCH", "COMAR", "COMDA", "COMCHEP", "CEVIBO", "COMSAM"], index=obtener_indice(["SELECCIONAR", "CON-C5", "CORCOL", "COMCH", "COMAR", "COMDA", "COMCHEP", "CEVIBO", "COMSAM"], datos_cargados.get("CENTRO_DE_MANDO")))
        unidad_vv = c1.selectbox("UNIDAD DE VV/104", lista_personal, index=obtener_indice(lista_personal, datos_cargados.get("UNIDAD_VV")))
            
        canal = c2.selectbox("CANAL DE ENTRADA", ["SELECCIONAR", "BOTÓN DE PÁNICO", "CLL-104", "ENLACE BCBPM", "ENLACE DIIP", "ENLACE DIP", "ENLACE DOT", "ENLACE MIBUS", "ENLACE MIGRACIÓN", "ENLACE MUPA", "ENLACE SENAFRONT CARIBE", "ENLACE SENAFRONT OCCIDENTAL", "ENLACE SENAFRONT ORIENTAL", "ENLACE SENAFRONT PMA. ESTE", "ENLACE SENAN", "OTRAS FUENTES", "RADIO FRECUENCIA", "VIDEO VIGILANCIA"], index=obtener_indice(["SELECCIONAR", "BOTÓN DE PÁNICO", "CLL-104", "ENLACE BCBPM", "ENLACE DIIP", "ENLACE DIP", "ENLACE DOT", "ENLACE MIBUS", "ENLACE MIGRACIÓN", "ENLACE MUPA", "ENLACE SENAFRONT CARIBE", "ENLACE SENAFRONT OCCIDENTAL", "ENLACE SENAFRONT ORIENTAL", "ENLACE SENAFRONT PMA. ESTE", "ENLACE SENAN", "OTRAS FUENTES", "RADIO FRECUENCIA", "VIDEO VIGILANCIA"], datos_cargados.get("CANAL_ENTRADA")))
        unidad_despacho = c2.selectbox("UNIDAD DE DESPACHO", lista_personal, index=obtener_indice(lista_personal, datos_cargados.get("UNIDAD_DESPACHO")))
        
        t_inicial = c3.time_input("HORA.INICIAL", value=parse_time(datos_cargados.get("T_INICIAL")), step=60)
        h_despacho = c3.time_input("HORA.DESPACHO", value=parse_time(datos_cargados.get("H_DESPACHO")), step=60)
        v_despacho = calcular_minutos(t_inicial, h_despacho)
        camara_id = c3.text_input("CAMARA/ID", value=datos_cargados.get("CAMARA_ID", ""))
        c3.number_input("V. DESPACHO (min)", value=v_despacho, disabled=True)
        
        h_atencion = c4.time_input("HORA.ATENCIÓN", value=parse_time(datos_cargados.get("H_ATENCION")), step=60)
        v_atencion = calcular_minutos(t_inicial, h_atencion)
        c4.number_input("V. ATENCIÓN (min)", value=v_atencion, disabled=True)
        
        h_cierre = c4.time_input("HORA.CIERRE", value=parse_time(datos_cargados.get("H_CIERRE")), step=60)
        ...
        c8, c9 = st.columns(2)
        tipo_inc = c8.selectbox("TIPO DE INCIDENTES", lista_maestra_a, index=obtener_indice(lista_maestra_a, datos_cargados.get("TIPO_INCIDENTE")))
        subtipo_inc = c9.selectbox("SUBTIPO DE INCIDENTES", lista_maestra_b, index=obtener_indice(lista_maestra_b, datos_cargados.get("SUBTIPO_INCIDENTE")))
        ...
        if modo == "PREVENTIVO":
            ...
            vehiculos_verificados = col_v.number_input("VEHICULOS VERIFICADOS", min_value=0, step=1, value=int(datos_cargados.get("VEHICULOS_VERIFICADOS", 0)))
            personas_verificadas = col_p.number_input("PERSONAS VERIFICADAS", min_value=0, step=1, value=int(datos_cargados.get("PERSONAS_VERIFICADAS", 0)))
        ...
        if modo == "POSITIVO":
            ...
            cierre_tipo = c_cierre1.selectbox("CIERRE TIPO", lista_maestra_a, index=obtener_indice(lista_maestra_a, datos_cargados.get("CIERRE_TIPO")))
            cierre_subtipo = c_cierre2.selectbox("CIERRE SUBTIPO", lista_maestra_b, index=obtener_indice(lista_maestra_b, datos_cargados.get("CIERRE_SUBTIPO")))
            
            st.write("Selección de Categorías:")
            cols_top = st.columns(3)
            p1 = cols_top[0].selectbox("P1", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P1")))
            p2 = cols_top[1].selectbox("P2", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P2")))
            p3 = cols_top[2].selectbox("P3", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P3")))
            
            cols_bottom = st.columns(3)
            p4 = cols_bottom[0].selectbox("P4", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P4")))
            p5 = cols_bottom[1].selectbox("P5", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P5")))
            p6 = cols_bottom[2].selectbox("P6", lista_pos, index=obtener_indice(lista_pos, datos_cargados.get("P6")))

        narrativa = st.text_input("REPORTE/NARRATIVA", value=datos_cargados.get("NARRATIVA", ""))
        link_video = st.text_input("ENLACE DE VÍDEO", value=datos_cargados.get("LINK_VIDEO", ""))
        
        # BOTÓN DINÁMICO
        texto_boton = "💾 Actualizar Cambios" if st.session_state.modo_pantalla == "EDICION" and st.session_state.get('caso_editar') else "Guardar Registro"
        submitted = st.form_submit_button(texto_boton)
        ...
        if submitted:
            ...
            if not latitud_actual: campos_faltantes.append("Ubicación en el Mapa") 
            ...
            if campos_faltantes:
                ...
                paquete_datos = {
                    "MODO": modo, "FECHA_EVENTO": str(fecha),
                    ...
                    "NARRATIVA": narrativa, "LINK_VIDEO": link_video, "LATITUD": str(latitud_actual),
                    "LONGITUD": str(longitud_actual), "VARIANZA_DESPACHO": v_despacho,
                ...
                try:
                    if st.session_state.modo_pantalla == "NUEVO" or not st.session_state.get('caso_editar'):
                        paquete_datos["FECHA_HORA"] = datetime.now(pytz.timezone('America/Panama')).strftime("%Y-%m-%d %H:%M:%S")
                        supabase.table("con_c5_datos_positivos").insert(paquete_datos, returning='minimal').execute()
                        st.success("✔️ Registro NUEVO guardado con éxito.")
                    else:
                        id_caso = st.session_state.caso_editar['id']
                        supabase.table("con_c5_datos_positivos").update(paquete_datos).eq("id", id_caso).execute()
                        st.success(f"✔️ Registro {id_caso} ACTUALIZADO con éxito.")
                        st.session_state.caso_editar = None
